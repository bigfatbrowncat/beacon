...
ResourceResponse Backend::ProcessRequest(const ResourceRequest& request) {
  ...
  if (request.getUrl() == "mem://logo.svg") {
    ...
    std::string htmlText =
        ...
        "type=\"submit\" value=\"Submit\"></input> <input id=\"textinput\" type=\"text\" "
        "class=\"growing\" value=\"Hello!\"></input>"
        "                        <p>Link: <a href=\"mem://index.html\">I am a "
        "hyperref</a></p>"
        "                        <p id=\"keydata\">Press a key...</p>"
        ...
        "               keydata.innerHTML += `keydown (key: '${event.key}', code: '${event.charCode}', mods: (`;"
        "               if (event.ctrlKey) { keydata.innerHTML += ' ctrl'; }"
        "               if (event.altKey) { keydata.innerHTML += ' alt'; }"
        "               if (event.shiftKey) { keydata.innerHTML += ' shift'; }"
        "               keydata.innerHTML += ` )<br />`;"
        "            }, true);"
        "            document.addEventListener('keypress', (event) => {"
        "               keydata.innerHTML += `keypress (key: '${event.key}', code: '${event.charCode}', mods: (`;"
        "               if (event.ctrlKey) { keydata.innerHTML += ' ctrl'; }"
        "               if (event.altKey) { keydata.innerHTML += ' alt'; }"
        "               if (event.shiftKey) { keydata.innerHTML += ' shift'; }"
        "               keydata.innerHTML += ` )<br />`;"
        ...
        "               keydata.innerHTML += `keyup (key: '${event.key}', code: '${event.charCode}', mods: (`;"
        "               if (event.ctrlKey) { keydata.innerHTML += ' ctrl'; }"
        "               if (event.altKey) { keydata.innerHTML += ' alt'; }"
        "               if (event.shiftKey) { keydata.innerHTML += ' shift'; }"
        "               keydata.innerHTML += ` )<br />`;"
